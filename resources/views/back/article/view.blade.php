...
@section('title') Zero|CMS - {{__('_basic.articles-view')}} @stop
@section('content')
    @include('widgets.content-msgInfo')
    <h2 class="page-header">{{__('_basic.articles-view')}}</h2>
            ...
            <div class="tab-pane active" id="tab_1">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="title" autocomplete="off"
                           value="{{$oArticle->title }}" readonly placeholder="标题">
                </div>
                @if($oArticle->flags != '')
                <div class="form-group">
                        <?php $tmp=explode(',',$oArticle->flags) ?>
                    <label>推荐位属性</label>
                ...
                <div class="form-group">
                    <label>正文
                        ...
                        <textarea class="form-control" id="ckeditor" readonly
                                  name="content">{{$oArticle->content }}</textarea>
                    @include('scripts.endCKEditor'){{-- 引入CKEditor编辑器相关JS依赖 --}}
                </div>
            </div><!-- /.tab-pane -->
            <div class="tab-pane" id="tab_2">
                @if($oArticle->outer_link)
                <div class="form-group">
                    <label>外链地址</label>
                    <input type="text" class="form-control" name="outer_link"
                           value="{{$oArticle->outer_link}}" >
                </div>
                @endif
                <div class="form-group">
                    <label>是否置顶
